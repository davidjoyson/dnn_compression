...
def run_experiment(get_data, num_classes, class_names, epochs, seeds, fine_tune_epochs, batch_size=128, model_dir=None, weight_decay=0.0,
    ...
    lit_baseline_max_seeds: optional cap for an expensive literature baseline;
      the first N seeds from `seeds` are used and its statistics report that N.
    """
    ...
    for seed_index, seed in enumerate(seeds):
        ...
        run_lit_baseline = (
            lit_baseline_fn is not None
            and (lit_baseline_max_seeds is None or seed_index < lit_baseline_max_seeds)
        )
        if run_lit_baseline:
            lit = lit_baseline_fn()
    ...
    return {
        ...
        "method_comparison": {
            ...
            "literature": {
                ...
                "num_seeds":              len(lit_acc_u_list),
                "accuracy_uncompressed":  _mean_safe(lit_acc_u_list),
                "f1_uncompressed":        _mean_safe(lit_f1_u_list),
