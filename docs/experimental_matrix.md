...
| Architecture ablation: trunk size | MIT-BIH and HAPT | Dendritic | `h1` sweep currently 16, 32, 64, 128; `h2` fixed at 32 | User-selected seeds; paper target 10 | Float32 and Snowflake accuracy; compression delta | Parameters and stored size | Desktop CPU | **Partial:** paired `h1`/`h2` sweep still required |
...
| Statistical equivalence | Each main dataset and applicable method | Dendritic and baselines | Compressed method paired with its own float32 model by seed | 10 paired seeds | Accuracy and macro-F1 CI/TOST, margin ±2 percentage points | Mean paired difference; 95% CI; both one-sided p-values | Not applicable | Accuracy and macro-F1 implemented; balanced-accuracy CI/TOST still planned |
| Resource fairness | Each main dataset | Dendritic; matched MLPs; compact ECG/HAR baseline | Float32 and applicable compressed variants | Profiling run plus repeated latency calls | Predictive metrics linked to resource point | Parameters; FLOPs/MACs; stored size; activation memory; latency | Desktop CPU | **Partial:** full identical profiling schema still needed for every baseline |
| Edge latency and memory | Current trained ECG/HAPT checkpoints where available | Dendritic variants | Float32 and storage-only/true-INT8 methods kept distinct | 50 warm-ups; 500 timed calls; batch 1 and full-batch in separate runs | Not an accuracy experiment | Latency mean/std; throughput; process memory; model size; speedup | Raspberry Pi 3 Model B, ARM Cortex-A53, QNNPACK | Implemented for existing checkpoints; CPU affinity/governor not pinned |
...
- Balanced-accuracy CI/TOST and complete baseline resource profiles remain open
  methodology items and must be completed before the corresponding paper claims.
