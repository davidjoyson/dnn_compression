...
|---|---|---|---|---|
| MIT-BIH ECG | 7.95 ms | 8.08 ms (0.98×) | 4.27 ms (**1.86×**) | 4.29 ms (**1.85×**) |
| INCART ECG  | 8.54 ms | 8.15 ms (1.05×) | 4.35 ms (**1.96×**) | 4.36 ms (**1.96×**) |
| HAPT        | 8.36 ms | 8.53 ms (0.98×) | 4.41 ms (**1.90×**) | 4.34 ms (**1.93×**) |

These are the controlled four-core runs for all three current datasets. HAR is
excluded because it is no longer part of the current evaluation set.
...
**Factorized architecture ablation:** the final three-seed, 20-epoch run changes
one axis at a time. ECG performs best at 8 branches, width 8 and a 64/32
trunk (85.13%); HAPT favors 12 branches (93.27%), width 2 (92.83%) and the
128/64 trunk (92.54%). Compression changes are small throughout, but the
architecture optima are dataset-specific and should be treated as exploratory.

**Component and regularization controls:** INT8 weight quantization alone is
effectively neutral in the ten-seed component run (85.34%→85.31% ECG;
93.01%→92.99% HAPT). Copying the first trained branch over all other branches
after training is destructive (38.21% ECG; 23.55% HAPT), so that result is
reported specifically as naive post-training topology replacement—not as a
test of a model trained with genuinely tied weights. In the separate
three-seed regularization control, weight decay (`1e-3`) reduces ECG accuracy
from 84.87% to 78.28% and HAPT from 92.93% to 92.09%; generic weight decay
therefore does not explain the near-lossless quantization result.

**Thermal:** the final ECG thermal protocol ran each of the 10 methods under a
five-minute continuous single-sample load, sampled temperature every two
seconds and used a 60-second cooldown between methods. Peak temperature across
all methods was **55.8°C**. Static INT8, Snowflake+Static INT8 and QAT INT8
sustained 229.2, 230.1 and 230.5 inferences/s respectively, compared with
123.2 inferences/s for Float32. The Pi reported `get_throttled=0x0` before and
after the run. This is evidence of bounded thermal behavior for these runs,
not a full thermal-stability claim: frequency, throttling flags and latency
were not sampled continuously.
...
| **Threading / CPU governor** | Reported runs use four PyTorch intra-op threads with affinity to cores 0–3. The governor was `performance`, with an observed 1.2–1.2 GHz range. Effective affinity, governor and frequency are stored in every CSV row. |
...
| **Memory (RSS)** | A separate untimed loop (`--memory-runs`, default 20) samples `/proc/self/status` after every forward and reports RSS before, observed peak, and delta. `gc.collect()` + `malloc_trim(0)` run first. Sampling stays outside the latency loop. One-time backend allocations are still shared across methods; fully isolated memory requires one process per method. |
| **Temperature** | `--temperature` records `vcgencmd measure_temp` immediately before and after every method without putting subprocess calls inside the timed loop. `thermal_test.py` remains the sustained-load test with periodic sampling and cooldown. |
