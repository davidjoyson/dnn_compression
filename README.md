...
| **Threading / CPU governor** | `benchmark_pi.py` defaults to one PyTorch intra-op thread (`--threads 1`), accepts an explicit CPU set such as `--cpu-affinity 0`, and can attempt `--set-governor performance` when permissions allow. Effective affinity, governor, and observed min/max frequency are stored in every CSV row; inability to change the governor is reported. |
