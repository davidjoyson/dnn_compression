...
**The motivating hypothesis — that the dendritic branching architecture is *more* robust to quantization than a conventional MLP — is not supported by the data.** A full 10-seed, 3-model (Dendritic vs. total-param-matched MLP vs. per-layer-matched MLP), 8-method comparison found an identical ~0.0026 mean accuracy-delta under compression across all three architectures (`docs/experiment_log.md`, 2026-07-19 entry). Dendritic's branches are measurably narrower and more weight-distinct than a structurally-equivalent control layer (2026-07-21 entry) — that structural property is real — but it doesn't translate into better compression tolerance. What does hold up: Dendritic's plain, uncompressed accuracy edge over both baselines, which is independent of quantization and survives the ECG leakage fix.
