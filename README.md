...
| HAPT | 12 | 72.50% ±5.71% | **73.01% ±5.78%** | **+0.51%** | EQUIV |

*(Confirmed 2026-07-27 against the current codebase, 3h27m run. ECG matched the prior `balance=True` numbers exactly. HAPT's absolute accuracy came in ~15-20pp lower than the number previously quoted here — that older figure was from a run that predated a 2026-07-20 rewrite of the core training code and was never re-checked against it; see the "Confirmation run results" subsection of the 2026-07-27 entry in `docs/experiment_log.md`. The TOST verdict and the Dendritic-vs-baselines robustness ordering are both unaffected by the correction.)*
...
**ECG and HAPT both train on oversampled, class-balanced data** (`balance=True`, restored as the default 2026-07-27) rather than their natural unbalanced distribution. This was flipped twice: unbalanced training was the default from 2026-07-21–2026-07-27 for cleaner TOST equivalence, faster training, and because oversampling didn't improve F1 — but that switch had an unexamined side effect: it also erased the one place Dendritic showed a real, reproducible quantization-robustness edge over both MLP baselines (Snowflake/Global/QAT, see Core Finding above). `balance=True` was restored once that was found, accepting the TOST/speed cost in exchange for the architecture-comparison result. Macro F1 and balanced accuracy remain low on both datasets' rare classes regardless of this choice — see `docs/experiment_log.md` for the full investigation of that separate problem.
...
Plots for all 10 benchmarked methods (not just the 4 above) are generated by `python plot_pi_benchmark.py` into `benchmark_pi_output/` — latency, memory, per-method speedup, a batch=-1-vs-batch=1 comparison, and a real-hardware compression-vs-speedup Pareto chart. Two things only visible there: **memory usage doesn't trade off consistently against speed** (true-int8 methods use more RAM than weight-only methods on HAR, but *less* on ECG — dataset-dependent, not a universal rule), and **Dynamic quantization's speedup verdict flips with batch size** — it's slower than Float32 at batch=-1 (0.44–0.60×) but faster at batch=1 (1.27–1.40×), so which one matters depends entirely on your deployment's batch size.

**Cross-dataset plots** (accuracy, F1, model size, and all 3 ablation studies compared across HAR/ECG/HAPT in a single chart each) are generated into `figures/combined/` on every run — see `docs/experiment_log.md`, 2026-07-22 entry. One finding only visible there: the architecture-size ablation's tiny-model (`h1=16`) collapse is stark on HAR and HAPT (accuracy craters with huge variance) but nearly invisible on ECG, because ECG's raw accuracy floors at its ~89% majority-class rate regardless of whether the model actually learned anything — the same masking effect documented in the F1 investigation above, now visible directly in the ablation chart.
